	define(["jquery","utils.cookie","jquery.dot","utils","jquery.base","jquery.validate"], function($,cookie,doT) {
		...
		var _init = function(){
			...
			//customInput('signType');
			//customInput('footType');
			customInput('capabilityCalScore');
			onLoadCapability();
			enablePopover();
		};
		$('#capabilityForm').myValidate({
			formCall:function(){debugger; onSubmit();},
			isAlert:false,
			formKey:false,
...
//						title: "提示",
		...
		setParam=function(capabilityCalScore){
			if(capabilityCalScore==1){
				$("#capabilityCustomizeScore").val("");
				$("#capabilityCustomizeScore").attr("readonly","readonly");
				$("#capabilityCustomizeScore").removeAttr('check-type');
				$("#capabilityCustomizeScore").removeAttr('required-message');
			}else if(capabilityCalScore==2){
				$("#capabilityCustomizeScore").removeAttr('readonly');
				$("#capabilityCustomizeScore").attr("check-type","number");
				$("#capabilityCustomizeScore").attr(" required-message","必填");
			}
		}
		validateNumCapability=function(){
			var flag = false;
			var numCapability=$.trim($("#numCapability").val())
			var url="/account/getAcountRelGroupCapability.html",data={
			};
			postAjaxRequest(url,data,function(result){
				var count = result.data.count;
				var capabilityLevel= result.data.capabilityLevel;
				if(count>0 && parseInt(numCapability)!=capabilityLevel){
					flag=true
				}
			}, false);
			return flag;
