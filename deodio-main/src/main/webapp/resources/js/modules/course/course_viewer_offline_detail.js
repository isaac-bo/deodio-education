...
//		 loadRelatedCourseRecommandDataList();
		 var href=$('.tab_area li[class=active] a').attr("href");
//         if(href="#relatedPage"){//导师预览页
//			 onLoadTraineeCourseRelatedInfo();
//		 }
...
//		$(".discuss_left").empty().append(template(result));
	...
	var onLoadCourseItemsInfo = function(){
		var url="/course/offline/item_list.html",data={
	...
	enrollCourse = function(){
		var data = {
			...
			postAjaxRequest(url, data, function(result) {
				if(result.status == 1){
					alertMsg("报名成功");
					go2Page("/course/course_viewer/offline_detail.html",'courseId='+$("#hiddenCourseId").val());
				}else{
					alertMsg("报名失败");
				}
			});
