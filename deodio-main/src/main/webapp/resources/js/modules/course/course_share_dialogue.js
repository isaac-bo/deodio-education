define([ "jquery", "utils.cookie", "jquery.dot", "pagination", "utils",
		...
		doT, paging) {
	// modal打开事件
	$('#courseShareModal').on('shown.bs.modal', function() {
		loadLecturerList();
	});
		...
		postAjaxRequest(url, params, function(result) {
			...
			if (result.status == 1) {
				paging.pageTemplateDiv(result, 
					"table_content_course_share",
					"right_content_course_share_template",
					"course_setting_course_share_panel", 
					"loadLecturerList");
			}
		});
	}
	...
	shareCourse = function() {
		...
		if(lecturer.length>0){
			...
			}, url = "/course/online/share_course_owner.html";
			postAjaxRequest(url, params, function(result) {
				if (result.status == 1) {
					alertMsg('该线上课程已经分享成功');
					reloadPageData();
				}
			});
		}else{
