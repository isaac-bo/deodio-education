	...
	var _init = function(){
		 ...
		 loadRelatedCourseRecommandDataList();
			...
			$('#submit').click(function () {
				...
				$("#contentId").val(courseId);
				var url = "/course/appraise/submit.html",
				data = $('#appraiseForm').serialize();
...
;
	...
	enrollCourse = function(){
		var data = {
				courseId : $("#hiddenCourseId").val(),
				itemType:$("#hiddenCourseType").val(),
				groupId:$("#hiddenGroupId").val()
			}, url = "/course/course_viewer/enroll_course.html";
	...
	openPreview = function(id,_type){
		...
		window.open("//"+httpCURL+"presentation/"+_methodUrl+"/preview.html?presentationId="+id+"&type=preview"); 
	};
	onViewExamination = function(quizId) {
		//window.open("//"+httpCURL+"/trainee/testQuiz.html?quizId="+quizId); 
		$("#_item_id").val(quizId);
		$("#quizLearnModal").modal("show");
	}
	//预览调查问卷
	onPreviewSurvey = function(surveyId){
		$("#_item_id").val(surveyId);
		$("#surveyLearnModal").modal("show");
	};
	enrollCourse = function(){
		var data = {
				courseId : $("#hiddenCourseId").val(),
	...
	onViewHomework=function(homeworkId){
		$("#_item_id").val(homeworkId);
		$("#hiddenCourseId").val($("#hiddenCourseId").val());
