...
<!-- 	            <li  style="width: 25%;"><a href="#medias" role="tab" data-toggle="tab">课件</a></li>
	        ...
	        <div class="catalog_box tab-content" style="overflow-y:auto;background-color: #4f5a72">
...
<!-- 	            <ul class="contents_list tab-pane " id="medias" role="tabpanel"> 
    ...
    <input value="${autoPlay}" type="hidden" id="auto_play_id">
</body>
...
<script type="text/javascript">
	require(['modules/presentation/presentation','modules/editor/editor','modules/presentation/presentation_package_preview' ], function(obj) {
...
{{~it.data:v:index}}
	<div class="conment_more_box" id="bg1">
		...
		<div class="conmenter_con pr18" style="cursor:pointer;" id="{{=v.id}}" onClick="javascript:onChoosePackageItem('{{=v.id}}','{{=index}}')" data-src="{{=v.packageUrl}}"  data-file-name="{{=v.packageGenerateName}}" data-package-ext="{{=v.packageExt}}">{{=v.packageName}}</div>
		<div class="btm_button first_conment"></div>
			
		<input type="hidden" name="countdown" id="{{=v.id}}" value="{{=v.countDown}}">							
