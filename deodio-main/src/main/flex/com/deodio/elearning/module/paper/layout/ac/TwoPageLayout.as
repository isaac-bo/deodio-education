package com.deodio.elearning.module.paper.layout.ac {
	...
	public class TwoPageLayout implements IPageLayout, IEventDispatcher {
		...
		public function renderPage(i:Number):void {
			...
			if (((!viewer.BusyLoading || viewer.DocLoader.IsSplit) && (!viewer.DocLoader.LoaderList[uloaderidx].loading)) && viewer.DocLoader.LoaderList != null && viewer.DocLoader.LoaderList.length > 0) {
				if (viewer.libMC != null && (viewer.numPagesLoaded >= viewer.PageList[i].dupIndex || viewer.DocLoader.IsSplit) && (viewer.DocLoader.LoaderList[uloaderidx] != null) && (viewer.DocLoader.LoaderList[uloaderidx].hasOwnProperty("content") && viewer.DocLoader.LoaderList[uloaderidx].content == null) || (viewer.DocLoader.IsSplit && viewer.DocLoader.LoaderList[uloaderidx].pageStartIndex != i + 1 && !viewer.DocLoader.LoaderList[uloaderidx].loading) || (viewer.DocLoader.LoaderList[uloaderidx].content.hasOwnProperty("framesLoaded") && viewer.DocLoader.LoaderList[uloaderidx].content != null && (viewer.DocLoader.LoaderList[uloaderidx].content.framesLoaded < viewer.PageList[i].dupIndex && !viewer.DocLoader.IsSplit))) {
		...
		public function mvPrev(interactive:Boolean=false):void {
			if (viewer.currPage > 1) {
				viewer.gotoPage(viewer.currPage - 2, 0);
			}
		}

		public function mvNext(interactive:Boolean=false):void {
			if (viewer.currPage + 2 <= viewer.numPages) {
				viewer.gotoPage(viewer.currPage + 2, 0);
			}
		}
