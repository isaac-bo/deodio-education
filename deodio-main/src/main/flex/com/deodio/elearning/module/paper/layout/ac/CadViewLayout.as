package com.deodio.elearning.module.paper.layout.ac {
	...
	public class CadViewLayout implements IPageLayout, IEventDispatcher {
		...
		public function initComponent(v:Viewer):Boolean {
			...
			_imgPreview.width=150;
			_imgPreview.height=150;
			_imgPreview.x=viewer.width - _imgPreview.width - 22;
			_imgPreview.y=viewer.height - _imgPreview.height - 22;
		...
		private function viewerSizeChanged(evt:Event):void {
			_imgPreview.width=viewer.width / 5;
			_imgPreview.height=_imgPreview.width / (viewer.width / viewer.height);
			_imgPreview.x=viewer.width - _imgPreview.width - 22;
			_imgPreview.y=viewer.height - _imgPreview.height - 22;
		}
		...
		public function mvPrev(interactive:Boolean=false):void {
			if (viewer.currPage > 1) {
				viewer.gotoPage(viewer.currPage - 1);
			}
		}

		public function mvNext(interactive:Boolean=false):void {
			if (viewer.currPage < viewer.numPages) {
				viewer.gotoPage(viewer.currPage + 1);
			}
		}
		...
		public function renderPage(i:Number):void {
			if ((!viewer.BusyLoading || viewer.DocLoader.IsSplit) && viewer.DocLoader.LoaderList != null && viewer.DocLoader.LoaderList.length > 0) {
				...
				if (!viewer.DocLoader.IsSplit && (viewer.libMC != null && viewer.numPagesLoaded >= viewer.PageList[i].dupIndex && viewer.DocLoader.LoaderList[uloaderidx] != null && viewer.DocLoader.LoaderList[uloaderidx].content == null || (viewer.DocLoader.LoaderList[uloaderidx].content != null && viewer.DocLoader.LoaderList[uloaderidx].content.framesLoaded < viewer.PageList[0].dupIndex))) {
		...
		private function redrawPreview(i:Number):void {
			...
			var rectPosXAdj:Number=_imgPreview.width * ((viewer.DisplayContainer.width * 0.96 - viewer.PageList[0].width) / viewer.DisplayContainer.width);
			rectPosXAdj=(rectPosXAdj < 0) ? 0 : rectPosXAdj / 2;
			var rectPosYAdj:Number=_imgPreview.height * ((viewer.DisplayContainer.height - viewer.PageList[0].height) / viewer.DisplayContainer.height);
			rectPosYAdj=(rectPosYAdj < 0) ? 0 : rectPosYAdj / 2;
