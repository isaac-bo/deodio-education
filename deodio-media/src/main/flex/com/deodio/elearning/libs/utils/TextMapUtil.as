...
package com.deodio.elearning.libs.utils {
	...
	public class TextMapUtil {
		...
		public static function alw(word:String, replacement:String, len:int=1):void {
			...
			langMap[idx]=new Array(3);
			langMap[idx][0]=word;
			langMap[idx][1]=replacement;
			langMap[idx][2]=len;
		}
			...
			for (var ci:int=0; ci < s.length; ci++) {
				if (s.charCodeAt(ci) > 10000) {
					...
					for (var ssi:int=ci + 1; ssi < map.length - 4; ssi++) {
						if (map.charCodeAt(ssi + 0) == s.charCodeAt(ssi + 0) && map.charCodeAt(ssi + 1) == s.charCodeAt(ssi + 1) && map.charCodeAt(ssi + 2) == s.charCodeAt(ssi + 2) && map.charCodeAt(ssi + 3) == s.charCodeAt(ssi + 3) && !foundMatch) {
